  ...
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS clientes (
        ...
        ultimo_pedido TIMESTAMP DEFAULT NOW(),
...
async function buscarCliente(numero) {
  if (!pool) return null;
  try {
    const res = await pool.query('SELECT * FROM clientes WHERE numero = $1', [numero]);
...
async function salvarCliente(numero, nome, contato) {
  if (!pool) return;
  try {
    await pool.query(`
      INSERT INTO clientes (numero, nome, contato, total_pedidos, ultimo_pedido)
      VALUES ($1, $2, $3, 1, NOW())
...
async function confirmarPedido(numero) {
  ...
  await salvarCliente(numero, d.nome || 'Cliente', d.contato || numero);

  // Montar resumo removendo campos internos
  const linhas = Object.entries(d)
    .filter(([k]) => !k.startsWith('_') && k !== 'etapa')
    .map(([k, v]) => `• *${capitalize(k)}:* ${v}`)
    .join('\n');

  const jaConhecia = d._clienteConhecido;
...
${jaConhecia ? 'Obrigada pela preferência de sempre na' : 'Obrigada pela preferência na'} *GS Studio Criativo*! 🛍️`);
